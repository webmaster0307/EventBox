...
const SignUp = ({ refetch }) => <SignUpFormWrapped refetch={refetch} />
  ...
  render() {
    ...
    return (
      <div className='sign-up-card__wrapper'>
        <Card title={i18n.t('signup')} headStyle={{ textAlign: 'center' }}>
          <Skeleton loading={loading} avatar>
            <Form onSubmit={this._handleSignUp} hideRequiredMark>
              {this.formFields().map((field) => {
                ...
                return (
                  <FormItem key={name} label={i18n.t(title)} colon={false}>
